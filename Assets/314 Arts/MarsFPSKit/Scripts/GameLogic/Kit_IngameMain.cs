    ...
    public class Kit_IngameMain : NetworkBehaviour
    {
        ...
        void Update()
        {
            ...
            if (NetworkServer.active || NetworkClient.active)
            {
                ...
                {
                    if (inputSystem.pause) //Escape (for non WebGL), B (For the editor), M (For WebGL)
                    {
                        //Change state
                        ...
                        if (!pb)
                        {
                            ...
                            if (!currentPvEGameModeBehaviour.UsesCustomSpawn())
                            {
                                if (gameInformation.allSingleplayerGameModes[currentGameMode].CanSpawn(player))
                                {
                                    ...
                                    if (spawnLocation)
                                    {
                                        ...
                                        NetworkServer.ReplacePlayerForConnection(sender, go, ReplacePlayerOptions.KeepActive);
                                    }
                                }
                            }
                            else
                            {
                                ...
                                if (playerGameObject)
                                {
                                    ...
                                    pb.isBot = false;
                                    pb.id = player.id;
                                    NetworkServer.ReplacePlayerForConnection(sender, playerGameObject, ReplacePlayerOptions.KeepActive);
                                }
                            }
                        }
                        break;
